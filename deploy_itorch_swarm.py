...
parser.add_argument('--nb-inst-type', default='m3.medium')
parser.add_argument('--nb-inst-price', default=0.03, type=float)
...
if args.existing_hub is None:
    hub_sir = poll(ec2, req_instances(ec2, args.hub_inst_type, args.hub_inst_price,
                                      sec_groups=['sg-0dbeee69'], init=HUB_INIT)
                  )
...
print('hub ip: %s' % hub_ip)

dev_map = BlockDeviceMapping()
dev_map['/dev/xvda'] = BlockDeviceType(size=20)

nb_sirs = poll(ec2, req_instances(ec2, args.nb_inst_type, args.nb_inst_price,
